...
# uart = serial.Serial(
#     "COM7", 115200
# )  # 在 windows 下控制电机测试，相应的输入连接的COM口和波特率

# linux系统下
result = os.popen(
    "sudo ls -l /dev/ttyACM*"
).read()  # 在 jetson nano（ubuntu）及树莓派（raspbian）下控制电机测试，相应的输入连接的串口和波特率
com = result.split()[
    -1
    ...
    try:
        if id_num <= MOTOR_NUM:
            ...
            if READ_FLAG == 1 and rdata[1] == 2:
                ...
                data = rdata[5:]
                motor_state[id_num - 1][0] = (
                    uint_to_float((data[0] << 8) + data[1], P_MIN, P_MAX, 16) * RAD_DEG
                )
                motor_state[id_num - 1][1] = (
                    uint_to_float((data[2] << 8) + data[3], V_MIN, V_MAX, 16)
                    * RAD_S_R_MIN
                )
                motor_state[id_num - 1][2] = uint_to_float(
                    (data[4] << 8) + data[5], T_MIN, T_MAX, 16
                )
...
def set_angle(id_num=127, angle=0, speed=10, limit_cur=27):
    ...
    write_property(
        id_num=id_num, index=0x7017, value=speed * R_MIN_RAD_S, data_type="f"
    )
...
def set_speed(id_num=127, speed=10, limit_cur=27):
    ...
    write_property(id_num=id_num, index=0x7018, value=limit_cur, data_type="f")
    write_property(
        id_num=id_num, index=0x700A, value=speed * R_MIN_RAD_S, data_type="f"
    )
...
def set_torque(id_num=127, torque=0.1):
    ...
    write_property(
        id_num=id_num, index=0x7006, value=torque / TORQUE_CONSTANT, data_type="f"
    )
...
def impedance_control(id_num=127, pos=0, vel=0, tff=0, kp=0, kd=0):
    ...
    try:
        ...
        cmd_data = [0] * 2
        cmd_data[0] = (
            float_to_uint(tff, T_MIN, T_MAX, 16)
        ) & 0xFF  # 阻抗控制里都是高位在前，低位在后，不满足小端排序
...
if __name__ == "__main__":
    # # 电机测试
    # # 1.阻抗控制
    # impedance_control(id_num=1, pos=0, vel=0, tff=0, kp=0.5, kd=0.2)
    # time.sleep(2)
    # # 2.速度控制
    # set_speed(id_num=1, speed=50, limit_cur=27)
    # time.sleep(10)
    # 3.位置控制
    set_angle(id_num=1, angle=0, speed=100, limit_cur=27)
